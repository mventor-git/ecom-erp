...
| **Secrets / creds** | Hardcoded secrets, weak admin pw | **CONFIRMED.** `ADMIN_PASSWORD=admin123` in `.env`; real Kashier secret hardcoded in `_seed-kashier.cjs:11-12`, `_kashier-fix-probe.cjs:6`; `.env` is git-ignored but probe scripts violate it. | YES | P1 | `server/.env`, `server/_seed-kashier.cjs`, `server/_kashier-fix-probe.cjs` |
