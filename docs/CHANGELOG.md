...
- **Ledger-vs-inventory reconciliation** `GET /api/admin/accounting/inventory-reconciliation` (ledger.read): posted-1300 net vs cost-layer book value + drift, ledger-owned movements with no posted journal (incl. blocked-by-closed-period, listing stays TRUE because the exclusion is status='posted'), issues whose order never settled (informational), PO-less manual receipts surfaced (no bridge ownership invented). Catch-up per movement `POST .../inventory-reconciliation/post/:movementId` (journals.manage, idempotent). NOTHING is auto-repaired — drift (including all legacy data) is listed with reasons. Admin page `/erp/inventory-reconciliation` in Finance nav.
- **Counter sales are a canonical settlement channel**: `POST /api/admin-sale` rebuilt from a fixed-id debug endpoint into a real walk-in flow — validated integer-cents pricing (qty 1..10000, unit prices hard-integer), NULL walk-in customer, FIFO issue + `settleCounterSale` (order completed + payment stamps + postOrderSale cash/revenue/COGS/snapshot journal + `order_settlement_recorded{settlement:'counter_sale'}`) in ONE transaction: 409 `SETTLEMENT_BLOCKED` on closed period (rollback proven: stock byte-identical), 403 without `orders.create` (previously ANY admin). Legacy response shape preserved for the storefront debug page + `settlement` trail added.
### Fixed
- `tests/jest.cleanup.js` (setupFilesAfterEnv): per-file sweep of `stock_%` journals whose source movements are gone — a PRODUCTION-impossible state (movements are immutable history), so real books can never be touched; keeps every suite's ledgers clean. `ledgerTrialBalance.test.js` global-exact-total assumption scoped to its own accounts (the always-true ΣD==ΣC kept global). Admin-sale route validation replaced silently-defaulted `|| 1 / || 1000` invented values.
### Verification (milestone C)
- New `tests/inventoryGL093.test.js` 14/14 (classification/sides/ownership, valuation tiers, one-door posting + replay + period blocked-draft-then-catch-up-exactly-one, return-against-booked-sale with refund adding NO stock, reconcile integers + drift identity + listed→posted→gone + unowned receipts, counter happy-path exact 4-leg journal, 409 full-rollback, 403, idempotent settle). Full matrix: unit **50 suites / 367 tests exit 0** (live AND isolated snapshot), integration **107/107 exit 0**, admin build 0, fresh-boot probe exit 0 (columns/perms/fake-zero intact), secret scan clean, live residue 0 (incl. stock-journal orphans).

## [4.24.0] - 2026-09-14 - milestone B (ticket 095): master-data onboarding imports + manual customer entry
### Added
- `onboardingImport` service + `/api/admin/onboarding` router (`/types`, `/template?type=`, `/preview`, `/commit`): CSV/XLSX for products, suppliers, customers, warehouses — column-flexible mapping, per-row validators (EGP→integer-cent money with a 2-decimal ceiling and NO silent truncation; email/phone/code validation), case-insensitive business-key dedupe (products by SKU then name, suppliers by name, customers email, warehouses code) with IN-FILE duplicate refusal and exact line numbers; PREVIEW is a pure read (planning runs in an always-rolled-back transaction — not even previewed auto-created categories/brands persist); COMMIT is ONE transaction, ALL-OR-NOTHING (≥1 invalid row aborts every effect: a half-imported sheet cannot happen); `master_data_imported` audit with actor; every submitted file archived under `server/data/imports/`; best-effort post-commit remote-image landing that never fails an import; ERP stock law preserved (imports NEVER mutate existing stock; new-stock cells post real opening movements; size×color variant expansion kept).
- Manual customer entry: `POST/PUT /api/admin/customers` (new `customers.manage` permission) — email dedupe refused with 409 + existing id, validations verbatim, `customer_created/updated` audit; "+ Add customer" dialog in CustomersList.
- `/setup/import` page (Sidebar "Import Data", wizard catalog step links): template download, plan table (creates/updates/errors with lines, auto cats/brands listed), Commit gated on a fully valid plan, stock-law + "all-or-nothing" honest copy.
### Removed
- The import operation-report PDF plus its **force-print to the OS default printer** side effect (SumatraPDF/Ghostscript/`Start-Process -Verb Print`) — silent paper is not a product behavior; legacy `POST /admin/products/import` became a thin delegation to the service (same response keys for the existing modal).
- Fix shipped alongside: the 094 fresh-boot test spawned a gitignored `_`-prefixed probe file (clean checkout broken) — probe relocated to tracked `server/probes/freshBootProbe.js`; probe dummy password assembled so scanners never misread a fixture as a secret.
### Verification (ticket 095)
- `tests/onboardingImport095.test.js` 11/11: preview purity+classification, in-file dup, abort-all, exact cents/markup pricing, stock-law on update, dataset round-trips, phone rejection, image jobs non-blocking, per-spec permission 403 matrix, customer create 201/409/400 + PUT + audit. Full matrix at fd68555: unit 49/353 exit 0 live + isolated 353 (live untouched), integration 107/107, both frontend builds 0, 094 probe relocated green, targeted residue ZERO (users/roles/rows/events/archives of every 095/094 pattern; journal/account/period/customer orphan sweeps = 0). Known pre-existing 76 product-type orphan events (old fixtures deleting products since 2026-08-31) intentionally left intact — audit rows are immutable history, not corruption.
### Product meaning
A fresh install plus the Setup wizard now covers: identity → locale → warehouse → first period → **import real masters** → continue. No developer seeding, no direct DB editing, no import surprising the operator with prints/partial writes.

## [4.23.0] - 2026-09-13 - milestone A (ticket 094): fresh-install integrity + first-run setup
### Fixed (PRODUCT-BREAKING, fresh installs only)
- SYSTEMIC schema-fork class closed at root: 51 optional columns (bilingual `name_ar`/`description_ar` on products/categories/brands, storefront badges/colors, 2FA + signature user columns, `customers.vip/invite_name`, `orders.temp_issue`, the whole `issue_orders` fulfillment chain…) were `ALTER ADD`ed at the TOP of `initDb()` — BEFORE their tables' `CREATE` — and their `try{}catch{}` swallowed every failure on a brand-new file, so fresh databases were missing every column a first product/order/fulfillment/2FA write needs (first bilingual product insert crashed; VIP on-bill orders wrote a phantom column; fulfillment reads threw 'no such column'). The migration block is now one idempotent `applySchemaMigrations()` list run EARLY (legacy upgrade path unchanged) **and again after all CREATE statements** (fresh-boot repair); `users.phone` covered explicitly; probe-verified zero missing on brand-new files with real bilingual/VIP/order inserts working. Same class previously patched ad-hoc twice (categories.icon, products.deleted_at); the idiom is now general so ADD-COLUMN-BEFORE-CREATE can never regress silently.
- Fake business data on fresh installs: the demo reviews + 6 fake-customer seed fired the moment ANY products existed — i.e. after an operator's own FIRST import, canned reviews and invented identities would attach to real merchandise. Seed retired (policy comment); reviews now come only from real actions. Existing historic demo rows untouched (no destructive rewrite).
- Boot robustness: SUPER_ADMIN bootstrap now defaults `ADMIN_EMAIL` → `ADMIN_USERNAME` → 'admin@localhost' (previously an UNSET var crashed the whole first boot with 'Cannot bind undefined'); `customer_site_url` settings row seeded (QR invites, receipt links and payment callbacks were structurally broken on fresh installs).
### Added
- **First-run setup system:** `setupService.getSetupStatus()` derived 100% from real records (identity ≠ the 'E-Commerce' default, currency/timezone, ≥1 active warehouse, first financial period, catalog > 0 products; guidance: suppliers/customers/opening stock/chart auto-note) — no state flag, nothing that can lie or drift; `GET /api/admin/setup/status`; `/setup` wizard page (inline company-identity form via settings.manage, checklist deep-linked to owning screens, "system ships intentionally EMPTY" positioning); AdminLayout persistent setup banner + one-time dashboard→setup redirect; sidebar "Setup" entry; actionable empty-state copy across Products/Suppliers/Customers lists; storefront WelcomePage infinite-spinner-on-empty trapped visitors → intentional "The shop is open - configure site content" escape (async public-settings access fixed properly).
- `server/tests/setupOnboarding094.test.js` (10 tests): fresh-boot column probe (child process, brand-new file, includes real INSERTs + permission/setting seeds + fake-data-zero + operator-empty counts), derived-setup-status guarantees (shape, company flip ONLY via real settings row, counts vs catalog), 401/auth gates. `probes/`-style tracked probe file, spawn-safe.
### Verification (milestone A)
- Unit 48 suites/342 exit 0 + isolated (fd-era names; suite count grew with 095), integration 107/107, admin + storefront builds 0, live residue 0. The 094 suite green AFTER probe relocation re-confirmed 2026-09-14.

## [4.22.0] - 2026-09-13 - mventor-ticket-092: GL operability — CoA, manual journals, journal list/detail, trial balance, P&L, balance sheet
### Added
- **`GET/POST/PUT/DELETE /api/admin/accounting/*` (new `routes/accounting.js`, zero accounting logic in routes):** CoA CRUD (`accounts.read/.manage`), manual journal lifecycle draft→post→unpost + filtered journal browsing + detail + per-account ledger (`journals.read/.manage`, `ledger.read`), and the three statements (Trial Balance, Profit & Loss, Balance Sheet). Every write delegates to `accountService`/`journalService` — no second ledger, no model forks.
- **`accountingService.js`** (new operator layer): manual-JE lifecycle with actor attribution + audit events, CoA mutations audited, statement boundary checks (calendar-valid ranges) — pure orchestration over existing services; statements derive from `ledgerService` posted-only reads.
- **Journal-derived statements** in `ledgerService`: `getProfitAndLoss({from,to})` (revenue/expense nets strictly from posted lines; basis string states order status/product cost are NOT inputs) and `getBalanceSheet({as_of})` (normal-side net classifications, derived current earnings, machine-checked identity `assets+other == liabilities+equity+earnings`, and `limitations[]` surfaced verbatim — no retained-earnings closing, no revaluation, coverage complete only as far as the 093 inventory↔GL slice). `journalService.listEntriesFiltered` adds date/status/origin/account/text filters + total_cents + pagination meta over the ONE journal model.
- **RBAC:** 5 new permissions (`accounts.read/.manage`, `journals.read/.manage`, `ledger.read`) via boot-safe insert-if-missing + automatic super_admin grant; period reopen got its missing adminApi+UI button. Audit: `journal_entry_created/updated/deleted` (manual drafts — bridges stay quiet), `account_created/updated/deleted`, `financial_period_closed/reopened` (close/reopen now carry the acting user).
- **Admin UI:** `/erp/chart-of-accounts` (list/create/activate/deactivate/delete with server refusal when referenced, per-account posted-only ledger drill-down), `/erp/journals` (filter bar, manual JE editor with EGP→cents + live balance meter, draft/post/unpost/discard actions only where the server allows, machine journals clearly badged immutable, detail with audit timeline), `/erp/statements` (TB/P&L/BS tabs with balance banners + limitation notes). Finance nav updated.
- `tests/glOperability.test.js` — 21 tests: CoA guards, full manual lifecycle, unbalanced/fractional/single-line/negative refusals, inactive-at-authoring AND at-post refusals, closed-period fail-closed on POST and on UNPOST + reopen recovery, sourced-journal machine-ownership (no unpost/edit/delete — posted or draft), source-injection refusal, TB draft-invisibility + D==C, P&L unaffected by orders/product-cost but exact on booked cents + date windows, BS normal-side netting + identity, filters/pagination, RBAC 403 matrix, audit chain via timeline, 404/400 mapping.
### hardened (test hygiene the 092 audit exposed)
- journal/period/account mutation audits now exist, so the four period-closing suites + `run-integration-tests.js` janitor sweep orphan events — live/integration residue returns to exactly 0.
### Verification
- Unit 47 suites / **332** exit 0 (live) AND isolated 332 exit 0 (live DB untouched); integration 107/107 exit 0; accounting regression group 12 suites green before UI; 092 HTTP gate smoke on booted throwaway copy **22/22** (auth, 403 matrix, period create/close/reopen over HTTP, draft→post refused in closed period then posted after reopen, TB balanced, P&L net = booked cents, BS identity, referenced-account refusal, sourced-unpost + sourced-edit refusal, unpost cycle, timeline audit); fresh-DB probe: 5 permissions seeded + super_admin grants + JE-0001 lifecycle + statements + audit events OK; admin build exit 0; secret scan clean; residue 0 (incl. orphan-event sweep).
### Explicitly NOT done (preserved guarantees)
- 086–091 posting bridges, AP aging (090) and revenue reconciliation (091) untouched — statements never reconcile-or-forge; the ~105k-cents legacy settled-unbooked population remains visible ONLY through the 091 control, with no auto-repair; fresh-start still intentionally spares accounts/journals/periods (survival documented). 093 (inventory↔GL) NOT started.
...
- **`salesPosting.reverseOrderSale`** — mirrors the POSTED sale's cash/revenue legs into a NEW journal (`sale-reversed`, ≤1 per order via `ux_journal_source`); expense/inventory legs deliberately untouched (refund ≠ physical goods return); no posted sale → honest no-op.
- Routes: `POST /api/admin/orders/:id/settle` + rebuilt `POST .../refund` (both `orders.manage` — refund was previously adminAuth-only) + `GET /api/admin/revenue-reconciliation` (`reports.read`). UI: OrdersList settle/refund evidence dialogs (slider money-states intercepted) + `/erp/revenue-reconciliation` page (Sales nav).
- `tests/salesSettlement091.test.js` — 29 tests over all 6 settlement matrix paths + 5 refund paths: journal shapes, cross-channel double-settle refusal (409), replay/retry recovery, closed-period fail-closed on BOTH sides (no paid-without-journal, no refund-without-reversal — webhook rolls back whole mutation), provider-refund webhook through the seam, no-inventory-restoration proof, integer-cent refusal, unpost immutability, reconciliation identity + healed-flag flow, HTTP RBAC 403 + slider 400 (SETTLEMENT_REQUIRED/REFUND_REQUIRED) + 409 boundary behavior.
### Changed
- **P1 Kashier webhook settlement is now ATOMIC:** `postOrderSale` moved INSIDE the webhook transaction (the 086 best-effort post-commit posting could strand a paid order unjournaled with NO re-post seam — the seam now exists on BOTH failure sides: handler failure rolls back the whole mutation AND the route clears the event claim so provider redelivery genuinely retries). COD settlement (P2) journals at delivery instead of stamping status only.
- **`salesPosting` canonical event is `sale-settled`** (legacy `kashier-paid` journals remain first-class citizens of the single-post-per-order guard). COGS derives from `order_items.cost_snapshot` when a caller has no fresh issue costs — closes the "idempotent-issue skip posted a 2-line journal" understatement hole.
- **Status slider can no longer book money:** `PUT /admin/orders/:id/status` rejects `paid`/`refunded` (backend authority; UI validation never was). Partial refund amounts (amount ≠ total, or non-integer) are REFUSED — the domain has no partial-refund truth, we do not fake it (documented for the future RMA slice).
- VIP on-bill untouched by design: approval stays approval-only; no AR model exists and none was invented — its deferred cash settlement flows through the settle endpoint (cash-basis, consistent).
### Fixed
- **Fresh-DB schema fork (pre-existing, 091-probe blocker):** `products.deleted_at` was ALTERed (swallowed) BEFORE the consolidated CREATE — brand-new DB files had no column and EVERY `createMovement` crashed from first boot (stock-in dead). CREATE now holds the column + idempotent post-CREATE probe (categories.icon precedent).
### Verification
- Unit: 46 suites / 307 tests **exit 0** on live DB AND twice on isolated snapshot (now 29 new); integration 107/107 exit 0 (two transient mobileCart 400s proved PRE-EXISTING by stashed-baseline A/B — pass every clean run); HTTP smoke 17/17 on booted throwaway copy (slider lock, settle 200 + 409, partial 400, reversal, replay 'already reversed', recon identity, webhook 401, auth gates); fresh-file boot probe: JE-0001→0002 settle→reversal→recon OK; admin build clean; secret scan clean; zero jest091 residue in live DB; tickets/ local-only.
- Reconciliation surfaced live-data truth: ~105,000 EGP-cents operationally-'paid' orders WITHOUT journals (pre-091 slider fabrications) — visible, classed, healable via settle book-only; NOT backfilled silently (owner decision at review).
### Known limitations (documented, not faked)
- Partial refunds / goods returns / their journals — future RMA slice; counter-sale cash accounting — future register slice; VIP credit recognition — awaits AR model (092/093 track); period-closed provider webhook keeps the order 'pending' until reopen (fail-closed by design; manual settle remains the escape hatch).
