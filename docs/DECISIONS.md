...
## ADR-017: Every sales settlement must be booked by a posted journal or never be paid (091)
**Context:** The settlement-path matrix proved the ledger was path-dependent: only the Kashier verified-payment webhook journaled (post-commit best-effort, so it could strand "paid-without-books"), while worker COD (payment_status='paid'), admin-slider 'paid', manual counter collections and ALL refund paths changed operational state with ZERO accounting effect — "paid" and "refunded" existed economically but not in the books.
**Decision:** One canonical sales journal per order across the three REAL settlement paths — P1 Kashier verified-payment webhook, P2 worker COD collected at delivery (status + proof-of-delivery), P3 evidence-backed manual/admin settlement — posted INSIDE the same nesting-aware transaction as the state change that makes it "paid": the financial mutation and the journal commit or roll back together. Idempotency + failure recovery are durable-state, never in-memory: the `order/<id>/sale-settled` `ux_journal_source` UNIQUE makes double-post structurally impossible (cross-channel too); a webhook-handler failure rolls back the ledger mutation AND clears the event claim so provider redelivery reprocesses (claim-delete pattern proven in N1). Manual settlement requires EVIDENCE (see below); refunds create a NEW mirrored journal, never edit/unpost. Reconciliation compares operational stamps against POSTED journal nets per order and exposes every disagreement — it never forces equality and never trusts `order.status`.
**Manual-paid policy chosen = B (explicit manual settlement source):** the server refuses "paid" from a status change (`PUT /orders/:id/status` rejects paid/refunded with SETTLEMENT_REQUIRED / REFUND_REQUIRED) and requires `POST .../settle` carrying actor (session) + method ∈ {cash,card,bank_transfer,wallet,other} + non-blank reason (<=300) + reference required for every method except cash; RBAC `orders.manage`; already-settled+booked => 409 ALREADY_SETTLED. An already-settled-but-UNBOOKED order may be booked book-only (self-heal of legacy slider-'paid').
**Alternatives considered:** A (require provider evidence only) — rejected: offline/COD/counter collections have no provider row to point at, so settlements would be blocked or faked provider data; C (forbid manual settlement entirely) — rejected: the business genuinely collects cash at the counter/door and an ERP must record it, just never as an evidence-less slider flick; post-commit best-effort posting (the 086 design) — rejected: it is exactly what left paid orders unjournaled with no recovery seam; a mutable revenue counter keyed off status — rejected (ADR-014); a second sales ledger — rejected.
**Consequences:** Revenue/COGS are recognized once per order on EVERY path; "paid" with no books is now a detectable (reconciliation-flagged, book-only-healable) anomaly rather than a normal state; closed periods fail-closed on settlements AND reversals including the provider webhook (delivered COD settlement now blocks until reopen — accepted owner-visible tradeoff, surfaced 409, no silent success). VIP on-bill approval and shipped-by-carrier status were deliberately NOT made financial (no AR/advances model exists — not invented). Physical goods return is kept separate from refund: the reversal mirrors cash/revenue only and NEVER restores inventory or reverses COGS. Partial refunds remain out of domain scope — the endpoint refuses amounts that are neither omitted nor equal to the full total (PARTIAL_REFUND_UNSUPPORTED), preserving truth until an RMA slice.

**ADDENDUM 2026-09-13 (verification gate): booked money can only move through the reversal seam.** The gate pass found two status-only money routes 091 had left open: the worker status endpoint accepted `paid`/`refunded` bodies (legacy mode allows those transitions — reproduced at file level: `refunded` persisted with zero journals), and EVERY path into `cancelled` (admin slider, worker, on-bill decline, trans-void direct UPDATE) could abandon or flip a BOOKED order while its sale journal stayed posted. Decision: ledger authority is enforced at the transition choke point — `transitionOrder` refuses `cancelled` whenever `hasPostedSale` (`LEDGER_BLOCKED`), the worker route carries the same SETTLEMENT_REQUIRED/REFUND_REQUIRED intent guards as admin, direct writers (mobile cancel, decline-onbill) pre-check the predicate, and a provider `trans-void` on a booked order marks the session VOIDED but NEVER cancels posted revenue (loud anomaly for admin refund handling). Consequence: once money is booked, the ONLY correction path is the immutable reversal; cancellation remains reserved for genuinely unsettled orders; `payment_verified` (configurable flow only, opt-in and OFF in production) is noted as a confirmation-gate state, not a ledger one.

## ADR-018: GL operator surface is a thin projection of the ONE journal model (092)
**Context:** After 091 every settlement/refund path was booked, but the general ledger had NO operator surface: no HTTP exposure of journals or accounts existed (075–080 shipped service-level only), manual corrections were impossible without SQL, and no journal-based statements existed — reportService "revenue/profit" remain operational status/cost views, which must never be confused with the books.
**Decision:** One new route family (`/api/admin/accounting/*`) + one operator service (`accountingService`) that ONLY orchestrates existing primitives: CoA CRUD → accountService (accountChart stays the sole skeleton home — no second chart); manual journals → journalService create/post/unpost (source forced NULL; posting inherits balance revalidation, active-account rules, integer cents and the fail-closed period gate; manual unpost keeps the 079 cycle and is now period-locked too; bridge-owned journals — posted OR stranded drafts — are machine-owned and refused human edit/delete at the choke point); Trial Balance → ledgerService verbatim; P&L and Balance Sheet are net-new READS over posted lines only (revenue/expense nets; normal-side asset/liability/equity nets; derived current earnings; arithmetic identity check `assets+other == liabilities+equity+earnings` is machine-verified and surfaced, with `limitations[]` naming what the model does NOT yet carry — retained-earnings closing, inventory revaluation/adjustment journaling, legacy unbooked settlements). RBAC: five fresh permissions (`accounts.*`, `journals.*`, `ledger.read`) seeded boot-safely; audit: manual draft lifecycle + CoA mutations + period close/reopen emit events (bridge postings stay audited through their existing journal_posted/source events).
**Alternatives considered:** exposing accounting logic in routes/React (rejected: server authority); a separate manual-entries table/flow (rejected: second journal model); making reportService "P&L" claim GL truth (rejected: it is status-based and stays labeled operational); closing/rolling entries or auto-reconciling the 105k legacy gap via statements (rejected: that is the 093 reconciliation slice; 092 must not force the books to match reality, only report the books); auto-repair of the settled_unbooked population (rejected again — 091 control owns visibility, owner owns triage).
**Consequences:** an accountant can actually run the ledger (post corrections, move balances, read statements) without touching SQL; all figures remain derived, not stored; statements are honest about coverage, so ledger-vs-physical divergence stays visible (093) instead of hidden; AP aging (090) and revenue reconciliation (091) remain the controls bridging journals to operational truth, untouched.
