...
**Consequences:** Report is honest (basis + limitations surfaced in the response itself: `aging_basis` field). True contractual due-date aging becomes a future slice when/if supplier invoices land. Control totals are machine-verified; mismatches fail the suite outright.
