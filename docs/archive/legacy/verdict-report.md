# Ecom-ERP ERP — Strengths, Weaknesses, Missing Features, Verdict
...
- **Weak admin password** in `server/.env`: `ADMIN_PASSWORD=********`
...
4. **HIGH — Rotate `ADMIN_PASSWORD`** in `server/.env` away from `********`
...
| `server/.env` | Live secrets: `ADMIN_PASSWORD=********` (rotate), `STRIPE_SECRET_KEY=sk_test_xxxx…` (dummy), Google + SMTP creds |
