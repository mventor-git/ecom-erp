# Ecom-ERP — Security Audit (closure)
...
- **NEW — production fail-fast guard** (`server/index.js`): refuses to start in `NODE_ENV==='production'` if `ADMIN_PASSWORD` is missing, <8 chars, or a known default (`********`/`password`/`admin`/…). Mirrors the existing `SESSION_SECRET` guard. Dev server unaffected (dev mode). **Closes the "known default production password" blocker.**
...
- `.gitignore` now covers env, DB, credentials, logs, and the secret-bearing scratch files. The live Kashier literal is out of source. "********" only remains as an **intentional deny-list** (index.js guard, `utils/passwordValidator.js`) and in **test/scratch** files — no production credential.
