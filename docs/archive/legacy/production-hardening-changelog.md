# Ecom-ERP — Production Hardening Changelog
...
### Docs
- `ECOM-ERP-VALUATION-SUPPLIER-AUDIT.md` (NEW) — inventory-costing + retail-pricing + supplier audit/design.
...
- **Backend**: returns/refunds, accounting/GL (structurally absent — won't fake), FIFO `cost_consumption` hardening, **secrets** (`ADMIN_PASSWORD=********`, Kashier secret in `_seed-kashier.cjs`/`_kashier-fix-probe.cjs`).
...
- Full detail: `ECOM-ERP-UI-UX-CLOSURE-REPORT.md`.
...
browser-verification boundary are documented in `ECOM-ERP-UI-UX-CLOSURE-REPORT.md`.
...
### Domain findings checkpoint (`ECOM-ERP-PAYMENT-DOMAIN.md`, NEW)
...
UI/UX/Arabic/RTL/design-system/reporting work (PARTIAL). Details in `ECOM-ERP-PAYMENT-DOMAIN.md`.
...
Arabic/RTL/design-system/a11y/responsive closure. See `ECOM-ERP-PAYMENT-DOMAIN.md`,
`ECOM-ERP-TEST-ACCOUNTS.md`.
