...
- **Deferred (pre-accounting queue, mostly resolved):** test-isolation: unit suites still mutate the live DB on `npm test` but self-clean; **F12/N-suite hardening** adds `ECOM_DB_PATH` + `run-isolated-tests.js` (`npm run test:isolated`) - integration 107/107 exit 0 and isolated runs 249/249 exit 0 twice. Backup retention remains; shared-DB on default `npm test` stays a documented trade-off. Period litter cleared 082; `po-item` leak fixed F12; N2 fixture litter purged after a crashed run; phase6 repaired+gated 083.
...
## Completed
- **mventor-ticket-093:** Inventory↔GL — one-door inventoryPosting bridge (openings→3000, shrinkage/gains→5100, booked-sale returns→COGS, vendor returns→AP; receipts/issues/transfers classification-exclusive), valuation cost→labeled stopgap→never-invented, ledger-vs-inventory reconciliation + idempotent catch-up posting, counter sales as canonical atomic settlement channel (409 full rollback, RBAC), jest.cleanup orphan sweep, ledgerTrialBalance scoped totals. 14/14 suite; matrix 50/367 live+isolated · 107 integration · builds 0 · fresh probe OK · residue 0.
- **mventor-ticket-095:** Master-data onboarding — preview→validate→dedupe→ALL-OR-NOTHING commit (products/suppliers/customers/warehouses; exact-cent money, in-file dup refusal, per-line errors, archive per import, image land-jobs non-blocking; stock law: updates never move inventory), /setup/import page, manual Add/PUT customer (`customers.manage`, 409 email dedupe + audit), legacy importer force-printer + PDF side-effect REMOVED, 094 probe relocated to tracked `probes/` (clean-checkout spawn fixed). 11-test suite; matrix 49/353 exit 0 live+isolated, 107 integration, builds 0, residue 0.
- **mventor-ticket-094:** Fresh-install integrity + first-run setup — 51-column ADD-COLUMN-before-CREATE fork class fixed at root (idempotent replay after creates; probe-verified brand-new files incl. real bilingual/VIP inserts), fake customers/reviews seed retired, ADMIN_EMAIL boot crash + customer_site_url seed fixed, WelcomePage spinner trap removed, derived Setup wizard (`/setup`, banner, `GET setup/status`) + actionable empty states. Tests 10 (incl. spawn probe). 48/342, 107, builds 0.
