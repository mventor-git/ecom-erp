# Comfort Sign — Kashier Source-Level Production Audit
...
- **Refunds — GAP.** Comfort Sign reacts to `transaction-refund` webhook by setting `refund_status`/order `refunded`; there is **no Kashier refund API call** from the app, **no partial refund**, and **no refund idempotency** beyond the webhook event-key. No provider-confirmation round-trip (it trusts the webhook). **REMAINING.**
